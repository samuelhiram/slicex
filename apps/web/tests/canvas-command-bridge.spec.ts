...
import {
  applyCanvasCommand,
  createDraftTimelineDocument,
  resolveInitialViewportOriginDate,
} from "../src/lib/canvasCommandBridge";
import type { CanvasStoreSnapshot } from "@slicex/canvas";

function createPort(initialSnapshot: CanvasStoreSnapshot) {
  ...
  return {
    ...
    setViewport(viewport: NonNullable<CanvasStoreSnapshot["viewport"]>) {
      snapshot = {
        ...snapshot,
        viewport,
      };
    },
    setPlayheadAt(value: string | Date | null) {
      snapshot = {
        ...snapshot,
        playheadAt: value,
      };
    ...
    patchDocument(
      updater: (document: CanvasStoreSnapshot["document"]) =>
        CanvasStoreSnapshot["document"],
    ) {
      snapshot = {
        ...snapshot,
