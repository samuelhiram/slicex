...
export function dateToPixel(date, originDate, zoom) {
    if (!(zoom > 0)) {
        throw new Error("zoom must be greater than 0");
    }
    const dayDelta = (toUtcMidnightMs(date) - toUtcMidnightMs(originDate)) / MS_PER_DAY;
    return dayDelta * DAY_WIDTH_PX * zoom;
}
export function pixelToDate(pixel, originDate, zoom) {
    if (!(zoom > 0)) {
        throw new Error("zoom must be greater than 0");
    }
