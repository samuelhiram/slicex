import { automationPointFromScreen, clamp, getHorizontalScrollbarRect, getHorizontalScrollbarThumbRect, getTrackIdByIndex, getVerticalScrollbarRect, getVerticalScrollbarThumbRect, getTrackIndexById, normalizeRect, rectsIntersect, screenXToTime, screenYToTrackIndex, snapTime, } from "../playlist-core";
import { hitTestPlaylist } from "./hit-test.js";
...
function setCursor(host, hit, active) {
...
function getSelectedDragClips(core, clip) {
...
function selectClipsInMarquee(core) {
...
function setHoverFromHit(core, hit) {
...
function executeTrackMenuAction(core, action) {
    const state = core.getState();
...
export function createPlaylistInteractionController(host, core) {
    ...
    const handlePointerDown = (event) => {
        ...
        if (hit.kind === "automation-point") {
            const automationPoint = hit.clip.points.find((candidate) => candidate.id === hit.pointId);
            if (!automationPoint) {
                return;
            }
            core.setSelection({
    ...
    const handlePointerMove = (event) => {
        ...
        if (gesture.kind === "clip-drag") {
            const primary = gesture.originals.find((clip) => clip.id === gesture.primaryClipId);
            if (!primary) {
                return;
            }
            const currentTime = screenXToTime(state, point.x, metrics);
            const rawPrimaryStart = primary.start + (currentTime - gesture.startPointerTime);
        ...
        if (gesture.kind === "automation-point-drag") {
            const clip = state.clips.find((candidate) => candidate.id === gesture.clipId);
    ...
    const handleWheel = (event) => {
        ...
        if (event.ctrlKey || event.metaKey) {
            ...
            if (timelineX >= 0) {
                ...
                const pxPerBeat = clamp(state.viewport.pxPerBeat * zoomFactor, metrics.minPxPerBeat, metrics.maxPxPerBeat);
