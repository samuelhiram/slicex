...
function drawGridAndRuler(graphics, textLayer, presentation) {
    ...
    graphics
        .rect(layout.sceneRect.x, layout.sceneRect.y, layout.sceneRect.width, layout.sceneRect.height)
        .fill({ color: COLORS.background });
    graphics
        .rect(layout.trackHeaderRect.x, layout.trackHeaderRect.y, layout.trackHeaderRect.width, layout.trackHeaderRect.height)
        .fill({ color: COLORS.panel });
    graphics
        .rect(layout.rulerRect.x, layout.rulerRect.y, layout.rulerRect.width, layout.rulerRect.height)
...
function drawTrackRows(graphics, textLayer, presentation) {
    ...
    for (const row of presentation.trackRows) {
        ...
        graphics
            ...
            .fill({ color: rowColor });
        graphics
            .rect(row.headerRect.x, row.headerRect.y, row.headerRect.width, row.headerRect.height)
            .fill({
            color: row.index % 2 === 0 ? COLORS.panelHeaderA : COLORS.panelHeaderB,
        });
        graphics
            .rect(row.stripRect.x, row.stripRect.y, row.stripRect.width, row.stripRect.height)
            .fill({ color });
        graphics
            .moveTo(row.rowRect.x, row.rowRect.y + row.rowRect.height - 1)
            .lineTo(row.rowRect.x + row.rowRect.width, row.rowRect.y + row.rowRect.height - 1)
            .stroke({ color: COLORS.rowLine, width: 1 });
        graphics
            .moveTo(metrics.trackHeaderWidth - 0.5, row.rowRect.y)
            .lineTo(metrics.trackHeaderWidth - 0.5, row.rowRect.y + row.rowRect.height)
        ...
        addText(textLayer, row.isVirtual ? "Empty" : `Track ${row.index + 1}`, 16, row.rowRect.y + 34, {
            color: COLORS.textMuted,
            size: 11,
        });
...
function drawClip(graphics, textLayer, clipView) {
    ...
    graphics
        .roundRect(clipView.rect.x, clipView.rect.y, clipView.rect.width, clipView.rect.height, 4)
    ...
    graphics
        .rect(clipView.titleRect.x, clipView.titleRect.y, clipView.titleRect.width, clipView.titleRect.height)
        .fill({ color: COLORS.panel, alpha: 0.34 });
    graphics
        .rect(clipView.resizeLeftRect.x, clipView.resizeLeftRect.y, clipView.resizeLeftRect.width, clipView.resizeLeftRect.height)
        .fill({ color: COLORS.text, alpha: 0.2 });
    graphics
        .rect(clipView.resizeRightRect.x, clipView.resizeRightRect.y, clipView.resizeRightRect.width, clipView.resizeRightRect.height)
        .fill({ color: COLORS.text, alpha: 0.2 });
...
function drawScrollbars(graphics, presentation) {
    ...
    graphics
        .rect(horizontal.trackRect.x, horizontal.trackRect.y, horizontal.trackRect.width, horizontal.trackRect.height)
        .fill({ color: COLORS.scrollbarTrack, alpha: 0.96 })
        .stroke({ color: COLORS.rowLine, width: 1 });
    graphics
        .roundRect(horizontal.thumbRect.x, horizontal.thumbRect.y, horizontal.thumbRect.width, horizontal.thumbRect.height, 4)
        .fill({ color: COLORS.scrollbarThumb });
    graphics
        .rect(vertical.trackRect.x, vertical.trackRect.y, vertical.trackRect.width, vertical.trackRect.height)
        .fill({ color: COLORS.scrollbarTrack, alpha: 0.96 })
        .stroke({ color: COLORS.rowLine, width: 1 });
    graphics
        .roundRect(vertical.thumbRect.x, vertical.thumbRect.y, vertical.thumbRect.width, vertical.thumbRect.height, 4)
        .fill({ color: COLORS.scrollbarThumb });
    graphics
        .rect(presentation.layout.scrollbarCornerRect.x, presentation.layout.scrollbarCornerRect.y, presentation.layout.scrollbarCornerRect.width, presentation.layout.scrollbarCornerRect.height)
        .fill({ color: COLORS.panelStrong });
}
