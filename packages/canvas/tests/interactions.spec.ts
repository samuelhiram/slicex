...
import type { CanvasStoreSnapshot } from "../src/types";
import type { CanvasInteractionCommand } from "../src/interactions";
...
function createMockHost(width = 800, height = 480) {
  ...
  const host = {
    style: {} as { cursor?: string; touchAction?: string; userSelect?: string },
...
function createMockStore(snapshot: CanvasStoreSnapshot) {
  return {
    getState: () => snapshot,
  };
}

const baseSnapshot: CanvasStoreSnapshot = {
  document: {
    id: "timeline-1",
    tenantId: "tenant-1",
    title: "Demo",
...
describe("createCanvasInteractionController", () => {
  ...
  it("zooms around the cursor on wheel gestures", () => {
    ...
    if (commands[0]?.type === "viewport/set") {
      ...
      expect(commands[0].viewport.originDate).toBe(
