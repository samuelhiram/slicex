...
it("projects a timeline document into scene state", () => {
  ...
  expect(projection.viewport.originDate.toISOString()).toBe(
  ...
  expect(projection.playhead.width).toBe(1280);
  expect(projection.playhead.height).toBe(720);
  expect(
    (projection.playhead.playheadAt instanceof Date
      ? projection.playhead.playheadAt
      : new Date(projection.playhead.playheadAt ?? "")
    ).toISOString(),
